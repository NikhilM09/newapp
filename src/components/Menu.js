...
const Menu = () => {
    ...
    if (menuList.length === 0)
        ...
        const { name, cuisines, areaName, feeDetails, avgRating, totalRatingsString } = menuList[2]?.card?.card?.info
            ...
            <div className="container mt-4" style={{ width: "60%" }}>
                ...
                <MenuHeader {...menuList[2]?.card?.card?.info} />
                {
                    filteredData.map((filteredItem, index) =>
                        ...
                        // </div>
                        <NormalMenu 
                        key={filteredItem?.card?.card?.title}
                ...
                }
                {
                    nestedData.map(nestedItem =>
                        ...
                        nestedTitle={nestedItem?.card?.card?.title} 
